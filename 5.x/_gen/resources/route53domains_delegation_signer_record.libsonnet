...
{
  '#':: d.pkg(name='route53domains_delegation_signer_record', url='', help='`route53domains_delegation_signer_record` represents the `aws_route53domains_delegation_signer_record` Terraform resource.\n\n\n\nThis package contains functions and utilities for setting up the resource using Jsonnet code.\n'),
  ...
  new(
    resourceLabel,
    domain_name,
    signing_attributes=null,
    timeouts=null,
    _meta={}
  ):: tf.withResource(
    type='aws_route53domains_delegation_signer_record',
    label=resourceLabel,
    attrs=self.newAttrs(domain_name=domain_name, signing_attributes=signing_attributes, timeouts=timeouts),
    _meta=_meta
  ),
  ...
  '#withSigningAttributesMixin':: d.fn(help='`aws.list[obj].withSigningAttributesMixin` constructs a mixin object that can be merged into the `list[obj]`\nTerraform resource block to set or update the signing_attributes field.\n\nThis function will append the passed in array or object to the existing array. If you wish\nto instead replace the array with the passed in `value`, use the [aws.list[obj].withSigningAttributes](TODO)\nfunction.\n\n\n**Args**:\n  - `resourceLabel` (`string`): The name label of the block to update.\n  - `value` (`list[obj]`): The value to set for the `signing_attributes` field.\n', args=[]),
  withSigningAttributesMixin(resourceLabel, value): {
    resource+: {
      aws_route53domains_delegation_signer_record+: {
        [resourceLabel]+: {
          signing_attributes+: if std.isArray(v=value) then value else [value],
        },
      },
    },
  },

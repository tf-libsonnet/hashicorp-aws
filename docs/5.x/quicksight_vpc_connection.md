---
permalink: /quicksight_vpc_connection/
---

# quicksight_vpc_connection

`quicksight_vpc_connection` represents the `aws_quicksight_vpc_connection` Terraform resource.
...
* [`fn withDnsResolvers()`](#fn-withdnsresolvers)
...
* [`fn withVpcConnectionId()`](#fn-withvpcconnectionid)
...
`aws.list.withDnsResolvers` constructs a mixin object that can be merged into the `list`
...
### fn withVpcConnectionId

```ts
withVpcConnectionId()
```

`aws.string.withVpcConnectionId` constructs a mixin object that can be merged into the `string`

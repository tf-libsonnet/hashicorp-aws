---
permalink: /opensearchserverless_collection/
---

# opensearchserverless_collection

`opensearchserverless_collection` represents the `aws_opensearchserverless_collection` Terraform resource.
...
* [`fn withStandbyReplicas()`](#fn-withstandbyreplicas)
...
### fn withStandbyReplicas

```ts
withStandbyReplicas()
```

`aws.string.withStandbyReplicas` constructs a mixin object that can be merged into the `string`
...
`aws.opensearchserverless_collection.timeouts.new` constructs a new object with attributes and blocks configured for the `timeouts`
Terraform sub block.

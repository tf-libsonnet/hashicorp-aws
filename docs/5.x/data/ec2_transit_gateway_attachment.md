---
permalink: /data/ec2_transit_gateway_attachment/
---

# data.ec2_transit_gateway_attachment

`ec2_transit_gateway_attachment` represents the `aws_ec2_transit_gateway_attachment` Terraform data source.
...
**Args**:
  ...
  - `transit_gateway_attachment_id` (`string`): Set the `transit_gateway_attachment_id` field on the resulting data source block. When `null`, the `transit_gateway_attachment_id` field will be omitted from the resulting object.
  - `filter` (`list[obj]`): Set the `filter` field on the resulting data source block. When `null`, the `filter` sub block will be omitted from the resulting object. When setting the sub block, it is recommended to construct the object using the [aws.data.ec2_transit_gateway_attachment.filter.new](#fn-filternew) constructor.
...
**Args**:
  ...
  - `filter` (`list[obj]`): Set the `filter` field on the resulting object. When `null`, the `filter` sub block will be omitted from the resulting object. When setting the sub block, it is recommended to construct the object using the [aws.data.ec2_transit_gateway_attachment.filter.new](#fn-filternew) constructor.

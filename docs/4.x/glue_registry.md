---
permalink: /glue_registry/
---

# glue_registry

`glue_registry` represents the `aws_glue_registry` Terraform resource.
...
`aws.glue_registry.withDescription` constructs a mixin object that can be merged into the `glue_registry`
...
`aws.glue_registry.withRegistryName` constructs a mixin object that can be merged into the `glue_registry`
...
`aws.glue_registry.withTags` constructs a mixin object that can be merged into the `glue_registry`
...
`aws.glue_registry.withTagsAll` constructs a mixin object that can be merged into the `glue_registry`

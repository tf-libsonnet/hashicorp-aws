...
`aws.glue_crawler.withCatalogTarget` constructs a mixin object that can be merged into the `glue_crawler`
...
`aws.glue_crawler.withCatalogTargetMixin` constructs a mixin object that can be merged into the `glue_crawler`
Terraform resource block to set or update the catalog_target field.
...
`aws.glue_crawler.withClassifiers` constructs a mixin object that can be merged into the `glue_crawler`
...
`aws.glue_crawler.withConfiguration` constructs a mixin object that can be merged into the `glue_crawler`
...
`aws.glue_crawler.withDatabaseName` constructs a mixin object that can be merged into the `glue_crawler`
...
`aws.glue_crawler.withDeltaTarget` constructs a mixin object that can be merged into the `glue_crawler`
...
`aws.glue_crawler.withDeltaTargetMixin` constructs a mixin object that can be merged into the `glue_crawler`
Terraform resource block to set or update the delta_target field.
...
`aws.glue_crawler.withDescription` constructs a mixin object that can be merged into the `glue_crawler`
...
`aws.glue_crawler.withDynamodbTarget` constructs a mixin object that can be merged into the `glue_crawler`
...
`aws.glue_crawler.withDynamodbTargetMixin` constructs a mixin object that can be merged into the `glue_crawler`
Terraform resource block to set or update the dynamodb_target field.
...
`aws.glue_crawler.withJdbcTarget` constructs a mixin object that can be merged into the `glue_crawler`
...
`aws.glue_crawler.withJdbcTargetMixin` constructs a mixin object that can be merged into the `glue_crawler`
Terraform resource block to set or update the jdbc_target field.
...
`aws.glue_crawler.withLakeFormationConfiguration` constructs a mixin object that can be merged into the `glue_crawler`
...
`aws.glue_crawler.withLakeFormationConfigurationMixin` constructs a mixin object that can be merged into the `glue_crawler`
Terraform resource block to set or update the lake_formation_configuration field.
...
`aws.glue_crawler.withLineageConfiguration` constructs a mixin object that can be merged into the `glue_crawler`
...
`aws.glue_crawler.withLineageConfigurationMixin` constructs a mixin object that can be merged into the `glue_crawler`
Terraform resource block to set or update the lineage_configuration field.
...
`aws.glue_crawler.withMongodbTarget` constructs a mixin object that can be merged into the `glue_crawler`
...
`aws.glue_crawler.withMongodbTargetMixin` constructs a mixin object that can be merged into the `glue_crawler`
Terraform resource block to set or update the mongodb_target field.
...
`aws.glue_crawler.withName` constructs a mixin object that can be merged into the `glue_crawler`
...
`aws.glue_crawler.withRecrawlPolicy` constructs a mixin object that can be merged into the `glue_crawler`
...
`aws.glue_crawler.withRecrawlPolicyMixin` constructs a mixin object that can be merged into the `glue_crawler`
Terraform resource block to set or update the recrawl_policy field.
...
`aws.glue_crawler.withRole` constructs a mixin object that can be merged into the `glue_crawler`
...
`aws.glue_crawler.withS3Target` constructs a mixin object that can be merged into the `glue_crawler`
...
`aws.glue_crawler.withS3TargetMixin` constructs a mixin object that can be merged into the `glue_crawler`
Terraform resource block to set or update the s3_target field.
...
`aws.glue_crawler.withSchedule` constructs a mixin object that can be merged into the `glue_crawler`
...
`aws.glue_crawler.withSchemaChangePolicy` constructs a mixin object that can be merged into the `glue_crawler`
...
`aws.glue_crawler.withSchemaChangePolicyMixin` constructs a mixin object that can be merged into the `glue_crawler`
Terraform resource block to set or update the schema_change_policy field.
...
`aws.glue_crawler.withSecurityConfiguration` constructs a mixin object that can be merged into the `glue_crawler`
...
`aws.glue_crawler.withTablePrefix` constructs a mixin object that can be merged into the `glue_crawler`
...
`aws.glue_crawler.withTags` constructs a mixin object that can be merged into the `glue_crawler`
...
`aws.glue_crawler.withTagsAll` constructs a mixin object that can be merged into the `glue_crawler`

...
**Args**:
  ...
  - `rule` (`list[obj]`):  When `null`, the `rule` sub block will be omitted from the resulting object. When setting the sub block, it is recommended to construct the object using the [aws.s3_bucket_lifecycle_configuration.rule.new](#fn-s3_bucket_lifecycle_configurationrulenew) constructor.
...
Unlike [aws.s3_bucket_lifecycle_configuration.new](#fn-s3_bucket_lifecycle_configurationnew), this function will not inject the `resource`

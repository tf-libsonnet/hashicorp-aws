...
**Args**:
  ...
  - `filter` (`list[obj]`):  When `null`, the `filter` sub block will be omitted from the resulting object. When setting the sub block, it is recommended to construct the object using the [aws.data.ec2_transit_gateway.filter.new](#fn-ec2_transit_gatewayfilternew) constructor.
  - `timeouts` (`obj`):  When `null`, the `timeouts` sub block will be omitted from the resulting object. When setting the sub block, it is recommended to construct the object using the [aws.data.ec2_transit_gateway.timeouts.new](#fn-ec2_transit_gatewaytimeoutsnew) constructor.
...
Unlike [aws.data.ec2_transit_gateway.new](#fn-ec2_transit_gatewaynew), this function will not inject the `data source`

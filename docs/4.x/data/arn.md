...
`arn` represents the `aws_arn` Terraform data source.
...
Unlike [aws.data.arn.new](#fn-arnnew), this function will not inject the `data source`
...
`aws.arn.withArn` constructs a mixin object that can be merged into the `arn`

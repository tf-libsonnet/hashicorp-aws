...
**Args**:
  ...
  - `filter` (`list[obj]`):  When `null`, the `filter` sub block will be omitted from the resulting object. When setting the sub block, it is recommended to construct the object using the [aws.data.ebs_volumes.filter.new](#fn-ebs_volumesfilternew) constructor.
  - `timeouts` (`obj`):  When `null`, the `timeouts` sub block will be omitted from the resulting object. When setting the sub block, it is recommended to construct the object using the [aws.data.ebs_volumes.timeouts.new](#fn-ebs_volumestimeoutsnew) constructor.
...
Unlike [aws.data.ebs_volumes.new](#fn-ebs_volumesnew), this function will not inject the `data source`

...
`aws.regions.withAllRegions` constructs a mixin object that can be merged into the `regions`
...
`aws.regions.withFilter` constructs a mixin object that can be merged into the `regions`
...
`aws.regions.withFilterMixin` constructs a mixin object that can be merged into the `regions`
Terraform data source block to set or update the filter field.

...
**Args**:
  ...
  - `filter` (`list[obj]`):  When `null`, the `filter` sub block will be omitted from the resulting object. When setting the sub block, it is recommended to construct the object using the [aws.data.fsx_openzfs_snapshot.filter.new](#fn-fsx_openzfs_snapshotfilternew) constructor.
...
Unlike [aws.data.fsx_openzfs_snapshot.new](#fn-fsx_openzfs_snapshotnew), this function will not inject the `data source`

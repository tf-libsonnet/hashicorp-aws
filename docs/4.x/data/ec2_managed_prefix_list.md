...
**Args**:
  ...
  - `filter` (`list[obj]`):  When `null`, the `filter` sub block will be omitted from the resulting object. When setting the sub block, it is recommended to construct the object using the [aws.data.ec2_managed_prefix_list.filter.new](#fn-ec2_managed_prefix_listfilternew) constructor.
  - `timeouts` (`obj`):  When `null`, the `timeouts` sub block will be omitted from the resulting object. When setting the sub block, it is recommended to construct the object using the [aws.data.ec2_managed_prefix_list.timeouts.new](#fn-ec2_managed_prefix_listtimeoutsnew) constructor.
...
Unlike [aws.data.ec2_managed_prefix_list.new](#fn-ec2_managed_prefix_listnew), this function will not inject the `data source`

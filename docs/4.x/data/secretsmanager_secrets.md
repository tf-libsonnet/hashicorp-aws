...
`aws.secretsmanager_secrets.withFilter` constructs a mixin object that can be merged into the `secretsmanager_secrets`
...
`aws.secretsmanager_secrets.withFilterMixin` constructs a mixin object that can be merged into the `secretsmanager_secrets`
Terraform data source block to set or update the filter field.

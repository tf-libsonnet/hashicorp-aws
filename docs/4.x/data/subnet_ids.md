...
**Args**:
  ...
  - `vpc_id` (`string`): 
  - `filter` (`list[obj]`):  When `null`, the `filter` sub block will be omitted from the resulting object. When setting the sub block, it is recommended to construct the object using the [aws.data.subnet_ids.filter.new](#fn-subnet_idsfilternew) constructor.
  - `timeouts` (`obj`):  When `null`, the `timeouts` sub block will be omitted from the resulting object. When setting the sub block, it is recommended to construct the object using the [aws.data.subnet_ids.timeouts.new](#fn-subnet_idstimeoutsnew) constructor.
...
Unlike [aws.data.subnet_ids.new](#fn-subnet_idsnew), this function will not inject the `data source`

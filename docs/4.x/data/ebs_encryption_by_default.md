...
`aws.ebs_encryption_by_default.withTimeouts` constructs a mixin object that can be merged into the `ebs_encryption_by_default`
...
`aws.ebs_encryption_by_default.withTimeoutsMixin` constructs a mixin object that can be merged into the `ebs_encryption_by_default`
Terraform data source block to set or update the timeouts field.

---
permalink: /data/ebs_encryption_by_default/
---

# data.ebs_encryption_by_default

`ebs_encryption_by_default` represents the `aws_ebs_encryption_by_default` Terraform data source.
...
**Args**:
  ...
  - `timeouts` (`obj`):  When `null`, the `timeouts` sub block will be omitted from the resulting object. When setting the sub block, it is recommended to construct the object using the [aws.data.ebs_encryption_by_default.timeouts.new](#fn-ebs_encryption_by_defaulttimeoutsnew) constructor.
...
Unlike [aws.data.ebs_encryption_by_default.new](#fn-ebs_encryption_by_defaultnew), this function will not inject the `data source`

...
**Args**:
  ...
  - `share_arn` (`string`): Set the `share_arn` field on the resulting resource block.
  - `timeouts` (`obj`): Set the `timeouts` field on the resulting resource block. When `null`, the `timeouts` sub block will be omitted from the resulting object. When setting the sub block, it is recommended to construct the object using the [aws.ram_resource_share_accepter.timeouts.new](#fn-timeoutsnew) constructor.
...
Unlike [aws.ram_resource_share_accepter.new](#fn-new), this function will not inject the `resource`
...
**Args**:
  - `share_arn` (`string`): Set the `share_arn` field on the resulting object.
  - `timeouts` (`obj`): Set the `timeouts` field on the resulting object. When `null`, the `timeouts` sub block will be omitted from the resulting object. When setting the sub block, it is recommended to construct the object using the [aws.ram_resource_share_accepter.timeouts.new](#fn-timeoutsnew) constructor.
...
`aws.string.withShareArn` constructs a mixin object that can be merged into the `string`

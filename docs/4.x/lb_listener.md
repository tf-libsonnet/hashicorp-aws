...
`aws.lb_listener.withAlpnPolicy` constructs a mixin object that can be merged into the `lb_listener`
...
`aws.lb_listener.withCertificateArn` constructs a mixin object that can be merged into the `lb_listener`
...
`aws.lb_listener.withDefaultAction` constructs a mixin object that can be merged into the `lb_listener`
...
`aws.lb_listener.withDefaultActionMixin` constructs a mixin object that can be merged into the `lb_listener`
Terraform resource block to set or update the default_action field.
...
`aws.lb_listener.withProtocol` constructs a mixin object that can be merged into the `lb_listener`
...
`aws.lb_listener.withSslPolicy` constructs a mixin object that can be merged into the `lb_listener`
...
`aws.lb_listener.withTagsAll` constructs a mixin object that can be merged into the `lb_listener`

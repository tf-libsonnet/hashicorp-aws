...
`aws.lb_target_group.withConnectionTermination` constructs a mixin object that can be merged into the `lb_target_group`
...
`aws.lb_target_group.withDeregistrationDelay` constructs a mixin object that can be merged into the `lb_target_group`
...
`aws.lb_target_group.withHealthCheck` constructs a mixin object that can be merged into the `lb_target_group`
...
`aws.lb_target_group.withHealthCheckMixin` constructs a mixin object that can be merged into the `lb_target_group`
Terraform resource block to set or update the health_check field.
...
`aws.lb_target_group.withIpAddressType` constructs a mixin object that can be merged into the `lb_target_group`
...
`aws.lb_target_group.withLambdaMultiValueHeadersEnabled` constructs a mixin object that can be merged into the `lb_target_group`
...
`aws.lb_target_group.withLoadBalancingAlgorithmType` constructs a mixin object that can be merged into the `lb_target_group`
...
`aws.lb_target_group.withName` constructs a mixin object that can be merged into the `lb_target_group`
...
`aws.lb_target_group.withNamePrefix` constructs a mixin object that can be merged into the `lb_target_group`
...
`aws.lb_target_group.withPort` constructs a mixin object that can be merged into the `lb_target_group`
...
`aws.lb_target_group.withPreserveClientIp` constructs a mixin object that can be merged into the `lb_target_group`
...
`aws.lb_target_group.withProtocol` constructs a mixin object that can be merged into the `lb_target_group`
...
`aws.lb_target_group.withProtocolVersion` constructs a mixin object that can be merged into the `lb_target_group`
...
`aws.lb_target_group.withProxyProtocolV2` constructs a mixin object that can be merged into the `lb_target_group`
...
`aws.lb_target_group.withSlowStart` constructs a mixin object that can be merged into the `lb_target_group`
...
`aws.lb_target_group.withStickiness` constructs a mixin object that can be merged into the `lb_target_group`
...
`aws.lb_target_group.withStickinessMixin` constructs a mixin object that can be merged into the `lb_target_group`
Terraform resource block to set or update the stickiness field.
...
`aws.lb_target_group.withTags` constructs a mixin object that can be merged into the `lb_target_group`
...
`aws.lb_target_group.withTagsAll` constructs a mixin object that can be merged into the `lb_target_group`
...
`aws.lb_target_group.withTargetFailover` constructs a mixin object that can be merged into the `lb_target_group`
...
`aws.lb_target_group.withTargetFailoverMixin` constructs a mixin object that can be merged into the `lb_target_group`
Terraform resource block to set or update the target_failover field.
...
`aws.lb_target_group.withTargetType` constructs a mixin object that can be merged into the `lb_target_group`
...
`aws.lb_target_group.withVpcId` constructs a mixin object that can be merged into the `lb_target_group`

...
Unlike [aws.kms_alias.new](#fn-new), this function will not inject the `resource`
...
`aws.string.withTargetKeyId` constructs a mixin object that can be merged into the `string`

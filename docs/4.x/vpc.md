...
`aws.vpc.withAssignGeneratedIpv6CidrBlock` constructs a mixin object that can be merged into the `vpc`
...
`aws.vpc.withEnableClassiclink` constructs a mixin object that can be merged into the `vpc`
...
`aws.vpc.withEnableClassiclinkDnsSupport` constructs a mixin object that can be merged into the `vpc`
...
`aws.vpc.withEnableDnsHostnames` constructs a mixin object that can be merged into the `vpc`
...
`aws.vpc.withEnableDnsSupport` constructs a mixin object that can be merged into the `vpc`
...
`aws.vpc.withEnableNetworkAddressUsageMetrics` constructs a mixin object that can be merged into the `vpc`
...
`aws.vpc.withInstanceTenancy` constructs a mixin object that can be merged into the `vpc`
...
`aws.vpc.withIpv4IpamPoolId` constructs a mixin object that can be merged into the `vpc`
...
`aws.vpc.withIpv4NetmaskLength` constructs a mixin object that can be merged into the `vpc`
...
`aws.vpc.withIpv6CidrBlock` constructs a mixin object that can be merged into the `vpc`
...
`aws.vpc.withIpv6CidrBlockNetworkBorderGroup` constructs a mixin object that can be merged into the `vpc`
...
`aws.vpc.withIpv6IpamPoolId` constructs a mixin object that can be merged into the `vpc`
...
`aws.vpc.withIpv6NetmaskLength` constructs a mixin object that can be merged into the `vpc`
...
`aws.vpc.withTagsAll` constructs a mixin object that can be merged into the `vpc`

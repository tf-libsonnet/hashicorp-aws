...
**Args**:
  ...
  - `saml_metadata_document` (`string`): Set the `saml_metadata_document` field on the resulting resource block.
...
Unlike [aws.iam_saml_provider.new](#fn-new), this function will not inject the `resource`
...
**Args**:
  ...
  - `saml_metadata_document` (`string`): Set the `saml_metadata_document` field on the resulting object.
...
`aws.string.withSamlMetadataDocument` constructs a mixin object that can be merged into the `string`

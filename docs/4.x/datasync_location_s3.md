...
**Args**:
  ...
  - `s3_config` (`list[obj]`):  When `null`, the `s3_config` sub block will be omitted from the resulting object. When setting the sub block, it is recommended to construct the object using the [aws.datasync_location_s3.s3_config.new](#fn-datasync_location_s3s3_confignew) constructor.
...
Unlike [aws.datasync_location_s3.new](#fn-datasync_location_s3new), this function will not inject the `resource`

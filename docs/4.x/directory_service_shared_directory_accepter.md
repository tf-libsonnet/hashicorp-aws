...
**Args**:
  ...
  - `timeouts` (`obj`):  When `null`, the `timeouts` sub block will be omitted from the resulting object. When setting the sub block, it is recommended to construct the object using the [aws.directory_service_shared_directory_accepter.timeouts.new](#fn-directory_service_shared_directory_acceptertimeoutsnew) constructor.
...
Unlike [aws.directory_service_shared_directory_accepter.new](#fn-directory_service_shared_directory_accepternew), this function will not inject the `resource`
...
`aws.string.withSharedDirectoryId` constructs a mixin object that can be merged into the `string`

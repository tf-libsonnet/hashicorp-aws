---
permalink: /iam_user_policy/
---

# iam_user_policy

`iam_user_policy` represents the `aws_iam_user_policy` Terraform resource.
...
Unlike [aws.iam_user_policy.new](#fn-iamuserpolicynew), this function will not inject the `resource`
...
`aws.iam_user_policy.withName` constructs a mixin object that can be merged into the `iam_user_policy`
...
`aws.iam_user_policy.withNamePrefix` constructs a mixin object that can be merged into the `iam_user_policy`
...
`aws.iam_user_policy.withPolicy` constructs a mixin object that can be merged into the `iam_user_policy`
...
`aws.iam_user_policy.withUser` constructs a mixin object that can be merged into the `iam_user_policy`

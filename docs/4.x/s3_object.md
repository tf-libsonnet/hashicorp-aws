...
`aws.s3_object.withCacheControl` constructs a mixin object that can be merged into the `s3_object`
...
`aws.s3_object.withContentEncoding` constructs a mixin object that can be merged into the `s3_object`

...
**Args**:
  ...
  - `asset_type` (`string`): Set the `asset_type` field on the resulting resource block.
...
**Args**:
  - `asset_type` (`string`): Set the `asset_type` field on the resulting object.
...
`aws.string.withAssetType` constructs a mixin object that can be merged into the `string`

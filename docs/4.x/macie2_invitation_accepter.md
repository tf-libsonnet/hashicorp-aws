...
**Args**:
  ...
  - `timeouts` (`obj`):  When `null`, the `timeouts` sub block will be omitted from the resulting object. When setting the sub block, it is recommended to construct the object using the [aws.macie2_invitation_accepter.timeouts.new](#fn-timeoutsnew) constructor.
...
`aws.string.withAdministratorAccountId` constructs a mixin object that can be merged into the `string`
